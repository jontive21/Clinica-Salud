...
const Admision = {
    ...
    buscarPorId: async (id) => {
        ...
        try {
            conexion = await pool.getConnection();
            const [filas] = await conexion.query("SELECT * FROM admisiones WHERE id = ?", [id]);
    ...
    buscarActivasPorIdPaciente: async (idPaciente) => { 
        ...
        try {
            conexion = await pool.getConnection();
            const [filas] = await conexion.query("SELECT * FROM admisiones WHERE paciente_id = ? AND estado_admision = 'Activa' ORDER BY fecha_admision DESC", [idPaciente]);
            return filas;
        } catch (error) {
